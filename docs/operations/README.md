# Operations Documentation

| Topic | Document |
|---|---|
| Logging | [LOGGING_POLICY.md](LOGGING_POLICY.md) |
| Incident response | [INCIDENT_RESPONSE.md](INCIDENT_RESPONSE.md) |
| Rollback | [ROLLBACK_PLAN.md](ROLLBACK_PLAN.md) |
| Hotfix | [HOTFIX_PROCEDURE.md](HOTFIX_PROCEDURE.md) |
| Post-release monitoring | [POST_RELEASE_MONITORING.md](POST_RELEASE_MONITORING.md) |
| Change management | [CHANGE_MANAGEMENT.md](CHANGE_MANAGEMENT.md) |
| Maintenance policy | [MAINTENANCE_POLICY.md](MAINTENANCE_POLICY.md) |
| Maintenance index | [MAINTENANCE_INDEX.md](MAINTENANCE_INDEX.md) |
| Dependency policy | [DEPENDENCY_POLICY.md](DEPENDENCY_POLICY.md) |
| Database policy | [DATABASE_POLICY.md](DATABASE_POLICY.md) |
| Bug fixes and regression | [BUG_FIX_WORKFLOW.md](BUG_FIX_WORKFLOW.md), [REGRESSION_POLICY.md](REGRESSION_POLICY.md) |
| Pull requests and reviews | [PULL_REQUEST_POLICY.md](PULL_REQUEST_POLICY.md), [CODE_REVIEW_CHECKLIST.md](CODE_REVIEW_CHECKLIST.md) |
| TDLib updates | [TDLIB_UPDATE_POLICY.md](TDLIB_UPDATE_POLICY.md) |
| Database changes | [../architecture/REFACTORING_STATUS.md](../architecture/REFACTORING_STATUS.md) |
| Upload engine changes | [../performance/PERFORMANCE_GUIDE.md](../performance/PERFORMANCE_GUIDE.md) |
| UI changes | [../archive/reports/localization/LOCALIZATION_AUDIT.md](../archive/reports/localization/LOCALIZATION_AUDIT.md) |
| Architecture changes | [CHANGE_MANAGEMENT.md](CHANGE_MANAGEMENT.md) |
| Technical debt | [TECHNICAL_DEBT.md](TECHNICAL_DEBT.md) |
| Maintenance certification | [MAINTENANCE_CERTIFICATION.md](MAINTENANCE_CERTIFICATION.md) |
| Support | [SUPPORT_GUIDE.md](SUPPORT_GUIDE.md) |
| Diagnostics | [DIAGNOSTIC_GUIDE.md](DIAGNOSTIC_GUIDE.md) |
