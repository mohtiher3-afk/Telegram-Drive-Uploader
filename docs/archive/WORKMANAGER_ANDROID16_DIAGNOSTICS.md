...
The app records `WORKER_ENQUEUED` when WorkManager accepts a unique upload request. v1.0.14 also records one asynchronous state snapshot immediately afterward. This snapshot is diagnostic evidence only; it does not declare the upload successful. WorkManager is configured through `TelegramDriveApp : Configuration.Provider` and Hilt’s `HiltWorkerFactory`; the manifest removes the nested AndroidX Startup initializer so this configuration is used consistently.
...
Install the signed `v1.0.14` APK on a physical API 36 device, keep the device connected to a validated network, and create one small test upload. Export diagnostics immediately after creation, then again after one minute, and again after the upload finishes or fails. Confirm that `WORKER_STARTED` follows `WORKER_ENQUEUED`. If it does not, collect filtered Logcat for `WM-`, `WorkManager`, `UploadWorker`, `Hilt`, and `TelegramDrive`.
