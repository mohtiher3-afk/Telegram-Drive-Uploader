...
| `UploadWorker` | `data/src/main/java/com/telegramdrive/uploader/data/upload/worker/UploadWorker.kt` | WorkManager execution | Updates Room | Queue observes Room | Owns preflight, result handling, retry result | Receives engine events indirectly | Completion requires terminal engine success; process/restart runtime behavior is not verified. |
