...
description: >
  Complete Python package management system. PROACTIVELY activate for: (1) uv package manager
  (10-100x faster), (2) pyproject.toml configuration, (3) Virtual environment setup,
  (4) Dependency management with uv.lock, (5) Ruff linting and formatting, (6) src layout
  project structure, (7) Publishing to PyPI, (8) pip and requirements.txt.
...
auto_load:
  enabled: true
  triggers:
    keywords: [uv, pip, package, dependency, pyproject, venv, virtual, install]
    file_patterns: ["pyproject.toml", "requirements*.txt", "setup.py"]
  priority: high
