...
name: python-asyncio
description: >
  Complete Python asyncio system. PROACTIVELY activate for: (1) async/await syntax,
  (2) asyncio.gather for concurrent execution, (3) TaskGroup (3.11+), (4) Semaphores
  for rate limiting, (5) Timeouts with asyncio.timeout, (6) Producer-consumer with Queue,
  (7) Async generators and context managers, (8) uvloop performance, (9) Common async gotchas.
...
auto_load:
  enabled: true
  triggers:
    keywords: [async, await, asyncio, concurrent, coroutine, gather, TaskGroup]
    file_patterns: ["*.py"]
  priority: high
